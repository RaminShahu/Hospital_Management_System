...
int main() {
  ...
  while (exit == false) {
    ...
    // code
      ...
      std::cout << "Enter the what type of ward you want to search(GW or PW): ";
      std::string wardType;
      std::getline(std::cin, wardType);
      if (wardType == "GW") {
        ...
        for (int i = 0; i < hospital.numberOfPrivateWard; i++) {
          int number = hospital.privateWard[i].getRoomNumber();
          if (number == roomNumber1) {
            hospital.privateWard[i].dispInfo();
          }
        }
      } else {
        std::cout << "Enter a valid ward type.\n";
      }
